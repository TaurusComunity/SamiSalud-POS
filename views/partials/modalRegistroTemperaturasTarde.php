...
<div class="flex flex-col w-full h-full ">
    ...
    <!-- Modal Content-->
    <div>
        <p>Llena los campos <span class="text-azul font-bold">CUIDADOSAMENTE</span> para obtener un registro del formulario éxitosamente.</p>
    </div>
    ...
    <?php foreach ($this->data['registroTemperatura'] as $registroTemperatura): ?>
        <form action="<?php echo constant('URL'); ?>/registroTemperatura/newTemperatura" method="post">
                ...
                <div class="w-full h-12 flex gap-2">
                    <div class="w-full h-12 relative flex rounded-xl">
                        <input required=""
                            ...
                            id="id" type="text" name="temperatura" value="<?php echo $registroTemperatura['temperatura'] ?>" />
                        <label
                            ...
                            for="id">
                            Temperatura normal</label>
                    </div>

                    <div class="w-full h-12 relative flex rounded-xl">
                        <input required=""
                            class="peer w-full bg-transparent outline-none px-4 text-base rounded-xl bg-white border border-[#4070f4] focus:shadow-md"
                            id="farmaco" type="text" name="manana" value="<?php echo $registroTemperatura['manana'] ?>" />
                        <label
                            class="absolute top-1/2 translate-y-[-50%] bg-white left-4 px-2 peer-focus:top-0 peer-focus:left-3 font-light text-base peer-focus:text-sm peer-focus:text-[#4070f4] peer-valid:-top-0 peer-valid:left-3 peer-valid:text-sm peer-valid:text-[#4070f4] duration-150"
                            for="farmaco">
                            Temperatura zona mañana</label>
                    </div>

                    <div class="w-full h-12 relative flex rounded-xl">
                        <input required=""
                            class="peer w-full bg-transparent outline-none px-4 text-base rounded-xl bg-white border border-[#4070f4] focus:shadow-md"
                            id="stock" type="text" name="tarde" value="<?php echo $registroTemperatura['tarde'] ?>" />
                        <label
                            class="absolute top-1/2 translate-y-[-50%] bg-white left-4 px-2 peer-focus:top-0 peer-focus:left-3 font-light text-base peer-focus:text-sm peer-focus:text-[#4070f4] peer-valid:-top-0 peer-valid:left-3 peer-valid:text-sm peer-valid:text-[#4070f4] duration-150"
                            for="stock">
                            Temperatura zona tarde</label>
                    </div>
                </div>
                <?php endforeach; ?>
                <div class="mt-1">
